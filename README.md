...
1. Log into your local MySQL instance and create the database:
```sql
   CREATE DATABASE simple_pins_api;
```
...
3. Set your database credentials in your environment variables or the `app/core/config.py` file to match your local MySQL setup.
...
Note: On startup, the `DatabaseManager` will attempt to connect to MySQL. If the database is unavailable, Tenacity will automatically retry with exponential backoff before failing.
...
Additionally, a Postman Collection is included in the `docs/` folder of this repository for testing complex authentication flows and endpoint scenarios.-->
...
* `app/models/`: Internal data models and database schemas.
